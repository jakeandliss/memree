	...
	describe "PATCH update" do
		context "with no token found" do
			it "render the edit page" do
				patch :update, id: 'notfound', user: {password: 'newpass1', password_confirmation: "newpass1" }
				expect(response).to render_template('edit')
			end
		...
		context "wiht a valid token" do
			let(:user) { create(:user) }
			before {user.generate_password_reset_token!}

			it "updates the user's password" do
				digest = user.password_digest
					patch :update, id: user.password_reset_token, user: { password: 'newpass1', password_confirmation: "newpass1" }
					user.reload
			...
			it "shows success message" do
				patch :update, id: user.password_reset_token, user: { password: 'newpass1', password_confirmation: "newpass1" }
				expect(flash[:success]).to match(/has been updated/)
			end
			...
			it "redirects to the titles path" do
				patch :update, id: user.password_reset_token, user: { password: 'newpass1', password_confirmation: "newpass1" }
